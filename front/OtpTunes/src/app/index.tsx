...
import ArtistAndSongs from '../components/ArtistAndSongs';
import AlbumAndSongs from '../components/AlbumAndSongs';
...
export default function Index() {
  ...
  const [matchedSongId, setMatchedSongId] = useState<string>('');
  ...
  //   try {
  //     var request = new Request('http://localhost:3000/getallsongs');
  //     fetch(request)
  //     .then((result) => { return result.json(); })
  //     .then((data) => {
  //       setTitle(data.rows[0].display_title);
  //       setArtist(data.rows[0].display_artist);
  //       setAlbum(data.rows[0].display_album);
  //     });
  ...
  function uploadSong() {
    ...
    //     newArtistText: matchedArtistId? null : newArtist,
    //     albumId: matchedAlbumId? matchedAlbumId : null,
    //     newAlbumTitle: matchedAlbumId? null : newAlbum,
...
function getAlbum() {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <ActionButton title='Go to Search' onPress={goToSearch} />
    </ScrollView>
  );
}
